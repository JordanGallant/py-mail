...
def get_weather_forecast(coords={'lat': -33.918861, 'lon': 18.423300}): # default location at Cape Town
    try: # retrieve forecast for specified coordinates
        api_key = 'cc8d323eef0029e96bd4b753432dd4b8' # replace with your own OpenWeatherMap API key
...
if __name__ == '__main__':
    ...
    st_f = {'lat': -34.160538,'lon': 24.824107} # coordinates for St Francis bay
    forecast = get_weather_forecast(coords = st_f) # get Austin, TX forecast
    if forecast:
        print(f'\nWeather forecast for {forecast["city"]}, {forecast["country"]} is...')
        for period in forecast['periods']:
            print(f' - {period["timestamp"]} | {period["temp"]}°C | {period["description"]}')
